...
void streaming_thread_function(
        int gamepad_id,
        std::string stream_name_sampled, std::string stream_type_sampled, double sample_rate,
        std::string stream_name_events, std::string stream_type_events,
        std::atomic<bool> &shutdown)
{
    QGamepad gamepad(gamepad_id);

    // Stream for irregular button-change events: https://doc.qt.io/qt-5/qgamepad.html#signals
    lsl::stream_info evstream_info(stream_name_events, stream_type_events, 2, lsl::IRREGULAR_RATE, lsl::cf_int8); // TODO: unique_id
    ...
    lsl::stream_info sampstream_info(stream_name_sampled, stream_type_sampled, 6, sample_rate, lsl::cf_double64);
...
void LSLManager::linkStream(int gamepad_id,
                            ...
                            QString stream_name_events, QString stream_type_events)
{
    if (streamer)
    {
        ...
        bool b_connected = QGamepadManager::instance()->isGamepadConnected(gamepad_id);
        if (b_connected)
        {
            ...
            streamer = std::make_unique<std::thread>(
                        ...
                        gamepad_id,
                        stream_name_sampled.toStdString(), stream_type_sampled.toStdString(), sample_rate,
                        stream_name_events.toStdString(), stream_type_events.toStdString(),
                        std::ref(shutdown));
            shutdown = false;
            streamStatusChange(!shutdown);  // emit signal so GUI may update stream-status indicator(s)
        }
